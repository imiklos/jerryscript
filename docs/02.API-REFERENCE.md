...
The function extracts the value from an error. If the second argument is true
it will also remove the error and abort value from the API value.
...
```c
jerry_value_t
jerry_get_value_from_error (jerry_value_t *value, bool clear_error)
```
